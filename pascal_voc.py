...
class pascal_voc(object):
    def __init__(self, phase, fliped, rebuild=False):
        self.devkil_path = os.path.join(cfg.dataset)
        self.cache_path = cfg.pkl
        ...
        self.epoch = 1     
        #self.gt_labels = None
        self.gt_labels = self.combine_imdb()
        self.num_gtlabels = len(self.gt_labels)
    
    def combine_imdb(self):
        """ combine VOC2007 imdb and VOC2012 imdb"""
        gt_labels = [self.prepare()]
...
#                gt_labels_cp[idx]['boxes'][:,[0,2]] = gt_labels_cp[idx]['image_size'][0] - gt_labels_cp[idx]['boxes'][:,[0,2]][:,::-1]
    ...
    def load_labels(self):
        cache_file_tmp = os.path.join(self.cache_path)
        ...
        txtname = os.path.join(self.devkil_path, self.phase + '.txt')
            #self.flipped = False
    ...
    def load_pascal_annotation(self, index):
        ...
        format.
        """
        filename = os.path.join(self.devkil_path, 'xmls', index + '.xml')
        ...
        for ix, obj in enumerate(objs):
            ...
            gt_classes[ix] = cls 
            imname = os.path.join(self.devkil_path, 'pictures', index + '.jpg')
